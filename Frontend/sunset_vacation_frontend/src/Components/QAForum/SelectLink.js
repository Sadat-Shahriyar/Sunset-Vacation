...
export default function SelectLink(props){
    ...
    function ViewAllProperties(props){      
        ...
        return(
            ...
            {properties.map((property)=>(
                <Grid item xs={4} key={property.propertyID}>
               {showCard(property)}
                </Grid>
            ))}
            </Grid>
    ...
    return(
        <Box id="overlay2"  sx={{   display: displayFunc(),flexWrap: 'wrap',position:'fixed' }} >     
     
        <Paper elevation={6} sx={{ ":hover": "none", padding: 4,bgcolor:'antiquewhite' ,minHeight:500}}>
        <IconButton sx={{ml:"90%"}} onClick={(event)=>{props.setDisplay('none');setInputValue("");}} ><HighlightOffIcon /></IconButton>

...
export default function EditPost(props){
    ...
    const fetchQuestion = async() => {
        console.log('question id:',props.selectedQuestionForEdit);
        fetch(`http://localhost:8000/qa/getQuestion/`+`${props.selectedQuestionForEdit}/`, {
          method: 'GET',
          headers: {
          ...
          .then((response) => {
            ...
            setSelectedProperty(response.question.propertyList)              
          })
          .catch((err) => {
              alert(err.message);
          })
   ...
   function handlePost(event){
    ...
    const requestOptions = {
      ...
      fetch(`http://localhost:8000/qa/updateQuestion/`+`${question.questions_id}/` , requestOptions)
        .then(response => response.json())
        .then(data => {
